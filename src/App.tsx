...
export default function App() {
  ...
  const [confirmDelete, setConfirmDelete] = useState<{ sceneId: string; title: string } | null>(null);

  // useMemo で参照安定化 → useAutoSave のデバウンスが正しく動く
  ...
  useEffect(() => {
    ...
    if (stored) {
      ...
      fetch(SAMPLE_JSON)
        ...
        .catch(() => setScenes(migrateData({}).scenes));
    }
    setLoaded(true);
  }, []);
  ...
  const removeScene = (id: string) => {
    ...
    setConfirmDelete({ sceneId: id, title: sc?.title ?? '場面' });
  };
  ...
  const removePage = (sid: string, pid: string) => setScenes(p => p.map(s =>
    s.id === sid
      ? { ...s, pages: s.pages.filter(pg => pg.id !== pid).map((pg, i) => ({ ...pg, label: `${i + 1}P` })) }
      : s
  ));
  ...
  const setHeroPanel = (sid: string, pid: string, kid: string) => setScenes(p => p.map(s =>
    s.id === sid
      ? { ...s, pages: s.pages.map(pg => pg.id === pid ? { ...pg, heroId: pg.heroId === kid ? null : kid } : pg) }
      : s
  ));

  const addPanel = (sid: string, pid: string) => setScenes(p => p.map(s =>
    s.id === sid
      ? { ...s, pages: s.pages.map(pg => pg.id === pid ? { ...pg, panels: [...pg.panels, mkPanel()] } : pg) }
      : s
  ));

  const removePanel = (sid: string, pid: string, kid: string) => setScenes(p => p.map(s =>
    s.id === sid
      ? { ...s, pages: s.pages.map(pg => pg.id === pid ? { ...pg, panels: pg.panels.filter(k => k.id !== kid) } : pg) }
      : s
  ));
  ...
  const updatePanel = (sid: string, pid: string, kid: string, field: 'content' | 'emotion', value: string) =>
    setScenes(p => p.map(s =>
      s.id === sid
        ? { ...s, pages: s.pages.map(pg => pg.id === pid ? { ...pg, panels: pg.panels.map(k => k.id === kid ? { ...k, [field]: value } : k) } : pg) }
        : s
    ));
  ...
  const addLine = (sid: string, pid: string, kid: string) => setScenes(p => p.map(s =>
    s.id === sid
      ? { ...s, pages: s.pages.map(pg => pg.id === pid ? { ...pg, panels: pg.panels.map(k => k.id === kid ? { ...k, lines: [...k.lines, { id: newId(), speaker: '', dialogue: '' }] } : k) } : pg) }
      : s
  ));

  const removeLine = (sid: string, pid: string, kid: string, lid: string) => setScenes(p => p.map(s =>
    s.id === sid
      ? { ...s, pages: s.pages.map(pg => pg.id === pid ? { ...pg, panels: pg.panels.map(k => k.id === kid ? { ...k, lines: k.lines.filter(ln => ln.id !== lid) } : k) } : pg) }
      : s
  ));
  ...
  const updateLine = (sid: string, pid: string, kid: string, lid: string, field: 'speaker' | 'dialogue', value: string) =>
    setScenes(p => p.map(s =>
      s.id === sid
        ? { ...s, pages: s.pages.map(pg => pg.id === pid ? { ...pg, panels: pg.panels.map(k => k.id === kid ? { ...k, lines: k.lines.map(ln => ln.id === lid ? { ...ln, [field]: value } : ln) } : k) } : pg) }
        : s
    ));
  ...
  const doDelete = () => {
    ...
    setScenes(p => p.filter(s => s.id !== confirmDelete.sceneId));
