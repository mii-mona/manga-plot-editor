...
export function LayoutPickerModal({ currentLayoutId, currentPanelCount, onSelect, onClose }: Props) {
  ...
  others.forEach(t => { (grouped[t.count] ??= []).push(t); });
  ...
  return (
    <>
      ...
      <div style={{
        ...
        zIndex: 210, background: C.card, borderRadius: 16, padding: '20px 22px',
        maxWidth: 400, width: '90vw', boxShadow: '0 12px 40px rgba(0,0,0,0.15)',
        maxHeight: '80vh', overflowY: 'auto',
      }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 4 }}>
          <span style={{ fontFamily: fonts.display, fontSize: 15, fontWeight: 700 }}>コマ割りテンプレート</span>
          <button
            onClick={onClose}
        ...
        <button
          ...
          style={{ background: 'none', border: `1px dashed ${C.cardBorder}`, borderRadius: 6, padding: '4px 10px', fontSize: 11, color: C.textSub, cursor: 'pointer', fontFamily: fonts.body, width: '100%', marginBottom: 10, textAlign: 'center' }}
        >
          {showAll ? '▲ 閉じる' : '▼ 他のコマ数のレイアウトも見る'}
        </button>

        {showAll && Object.entries(grouped)
          .sort(([a], [b]) => Number(a) - Number(b))
          .map(([count, tpls]) => (
            <div key={count} style={{ marginBottom: 12 }}>
              <div style={{ fontSize: 10, fontWeight: 600, color: C.textSub, marginBottom: 6 }}>{count}コマ</div>
